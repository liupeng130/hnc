...
export function addNewSpecification(param){
  ...
  //删除无用数据
  delete param.keys;
  var paramJson = "platformAttributeParam="+JSON.stringify(param);
  debugger;
  return {
    types: [ADD_LOADING, ADD_SUCCESS, ADD_FAIL],
    promise: (client) => client.post('/item/platform/categoryAttribute/insertPlatformCategoryAttribute',{data:paramJson})
