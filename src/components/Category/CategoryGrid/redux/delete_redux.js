...
export function selectChangeStatus(data,record) {
  if(record.isCheck){
    ...
    data = setDataAttrByKey(data,record.key,record,'isCheck',false);//取消选中父节点，则取消选中的子节点
    if(record.lev>1) {//取销子节点的选中时取消父节点父节点的选择
      data = cancelSelect(data, record, 'isCheck', false);
    }
  }else{
    record.isCheck = true;
    data = setDataAttrByKey(data,record.key,record,'isCheck',true);
  }
  return {
...
export function batchCategoryDelete(data,UUID,platformId) {
  ...
  params.uuid = UUID;
  var checkData = getDeleteData(data);
  console.log(delData);
  var delData = getLevelDataByParentCid(checkData);
  var list = processData(data,delData);
...
export function categoryDelete(data,cids,key,type,UUID,platformId) {
  var list = getDataByKey(data,key,type);
  ...
  return {
    types: [DELETE, DELETE_SUCCESS, DELETE_FAIL],
    changeData:list.data,
    promise: (client) => client.post('/item/category/deletePlatformCategory',{data:params})
}
}
//删除的时候更改数据结构
