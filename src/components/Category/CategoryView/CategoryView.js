...
import { Modal, Table, Transfer, Button } from 'jcloudui';
...
export default class CategorySee extends Component {
  ...
  handleDeleteData(record) {
    const platformId = 2, uuid = this.props.getUUID(32, 16);
      this.props.handleDeleteData(record, this.props.brand.categoryId);
      this.setState({
        message: {
          height: '0',
          top: '-300px',
      ...
      this.props.Relieve(platformId, uuid, this.props.brand.categoryId, record.id)
        .then(
          (function () {
            console.log(this.props.brands.data);
          }).bind(this)
        );
  }
          ...
          (function (brands) {
            ...
            for (let i = 0, length = brands.data ? brands.data.length : 0; i < length; i++) {
              const data = {
                ...
                title: brands.data[i].brandNameCh,
                id: brands.data[i].id,
                address: '解除关联',
              };
              mockData.push(data);
  ...
  submitMock() {
    ...
    if(dataArr.length !== 0){
      ...
      this.props.Relation(platformId, uuid, this.props.brand.categoryId, brandData)
        .then(
          (function () {
            console.log(this.props.brands.data);
          }).bind(this)
  ...
  render() {
    const styles = require('./style/CategoryView.less');
    ...
    return (
      <div>
        ...
        >
          ...
          />
          <div className={styles.brandText} onClick={this.transferOpen.bind(this)}><span>关联新品牌</span>
            <div style={{marginTop: '1px', position: 'relative', overflow: 'hidden', height: (this.props.visibleView && this.visible ? this.state.message.height : '0'), background: '#fff', transition: '1s'}}>
              <div style={{position: 'absolute', top:  (this.props.visibleView && this.visible ? this.state.message.top : '-300px'), padding: '14px', transition: '1s'}}>
