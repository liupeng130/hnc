...
export default class RegionPriceEdit extends BaseComponent {
  ...
  initialAreaListState(){
    var states = this.props.RegionPriceEdit &&　this.props.RegionPriceEdit.initialdata && this.props.RegionPriceEdit.initialdata.map((k)=>{
        return false;
      });
    return states;
  }
  ...
  initialKeys(){
    var keys=new Array();
    var i=-1;
    var tmp = this.props.RegionPriceEdit && this.props.RegionPriceEdit.initialdata && this.props.RegionPriceEdit.initialdata.map((k)=>{
        keys.push(++i);
      });
    return keys;
  }
  ...
  buildData(values){
    ...
    */
    return {uuid:this.props.uuid, skuId:this.props.skuId, totalPrice:values.totalPrice,priceType:this.props.priceType, priceList:priceList}
  }
  ...
  add(){
    ...
    /*let lastValue = 0
    if(keys.length>0){
      lastValue = keys[keys.length-1]+1;
    }*/
...
/*      [`areaTitle-${k}`]:'',
            ...
            <Row>
              ...
              <Col span={7}>
                {getFieldDecorator(`areaTitle-${k}`, {
                  ...
                  <Input disabled={true} maxLength="15" style={{ border: 0}} />
                )}
              </Col>
  ...
  render() {
    ...
    //-----------------main-----------------------
    return (
      <a className="ml10" onClick={()=>this.showModal()}>设置地域价
        <Modal
          title={'设置地域价'}
